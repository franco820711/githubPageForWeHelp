...
import { TestVideoDialog } from "../test";
  ...
  return (
    <>
      <TestVideoDialog
        ...
        isOpen={videoOpen}
        videoSrc={card.videoUrl ? getEmbedUrl(card.videoUrl) : ""}
      />
      <AnimatePresence>
        {open && (
      ...
      >
        ...
        <div className="relative z-40 p-8">
          ...
          <motion.p
            layoutId={layout ? `title-${card.title}` : undefined}
            className="text-white text-xl md:text-2xl font-semibold max-w-xs text-left [text-wrap:balance] font-sans mt-2 whitespace-pre-wrap"
          >
            {card.title}
          </motion.p>
        </div>
        ...
        {card.videoUrl && (
          ...
          >
            <div className="bg-white/80 backdrop-blur-sm p-2 rounded-full">
              <IconPlayerPlay className="w-6 h-6 text-black" />
            </div>
          </div>
        )}
        <BlurImage
